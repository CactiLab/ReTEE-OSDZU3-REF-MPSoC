/*
 * load_app — minimal SSA loader for BYOTEE RISC-V.
 *
 * Loads an ELF module onto the MicroBlaze RISC-V core via DRAM staging,
 * sends CMD_RUN, waits for completion, prints results, and unloads.
 *
 * Usage:  load_app [elf_path] [dram_addr_hex]
 *   elf_path   — path to the .elf file  (default: /tmp/embench.elf)
...
static int load_elf_dram(shared_ocm_t *ocm, const char *elf_path,
    ...
    printf("Waiting for firmware to load ELF...\n");
    while (!ocm->ready)
        continue;
...
int main(int argc, char *argv[]) {
    const char *elf_path  = "/tmp/embench.elf";
    uint32_t    dram_addr = 0x50000000;

    if (argc >= 2) elf_path  = argv[1];
    ...
    if (load_elf_dram(ocm, elf_path, dram_addr) < 0)
        return 1;
