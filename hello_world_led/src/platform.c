...
void arm_interrupt_handler(XIntc *intc) {
    ...
    uint32_t received_data = SHARED_OCM->data[0];
        
    // Optional: Clear ARM flag
    SHARED_OCM->arm_to_mb_flag = 0;
    
    // Optional: Send acknowledgment back
    SHARED_OCM->mb_to_arm_flag = 1;
    
    // Debug print (be careful with prints in ISR - keep short!)
    xil_printf("[m] IRQ from ARM! Cmd: 0x%08X, Data[0]: 0x%08X\r\n", received_command, received_data);
...
int init_platform()
{
    ...
    *led_data = 0;
    ...
    Status = XIntc_Initialize(&InterruptController, 0x41200000);
...
void debug_interrupt() {
    u32 isr = Xil_In32(InterruptController.BaseAddress + XIN_ISR_OFFSET);
    
    xil_printf("Interrupt Status Register: 0x%08X\n\r", isr);
    
    // Assuming 2 software interrupts
    if (isr & 0x01) print("  - Interrupt 0 (GPIO) pending\n\r");
    if (isr & 0x02) print("  - Interrupt 1 pending\n\r");
    if (isr & 0x04) print("  - Interrupt 2 pending\n\r");
}