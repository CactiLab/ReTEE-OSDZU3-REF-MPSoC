...
#pragma HLS LOOP_TRIPCOUNT min=1 max=16
        ...
        // Split into 2x iterations to avoid 2 BRAM reads per cycle (II=1)
        PW_PRELOAD_RQ:
        for (int idx = 0; idx < tile_oc * 2; idx++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=2 max=32
            int t = idx >> 1;
            int phase = idx & 1;
            int base_word = ((int)requant_base + (oc_base + t) * 8) / 4;
            ap_uint<32> rd = act_in[base_word + phase];
            if (phase == 0) {
                rq_mult_cache[t] = (int32_t_hls)rd;
            } else {
                rq_shift_cache[t] = (int8_t_hls)rd( 7,  0);
                rq_amin_cache[t]  = (int8_t_hls)rd(15,  8);
                rq_amax_cache[t]  = (int8_t_hls)rd(23, 16);
            }
        }

        // Process all spatial pixels against cached weights
        PW_PIX: for (int pix = 0; pix < spatial; pix++) {
#pragma HLS LOOP_TRIPCOUNT min=1 max=2304
...
// ============================================================
//  Depthwise 3x3 convolution — line-buffered, merged pipeline
//
//  Caches 3 input rows per channel tile into partitioned LUTRAM,
//  enabling DW_PARALLEL channels to be processed per cycle.
//
//  All output pixels for a channel tile are processed in a single
//  pipelined loop (DW_OW_COMPUTE).  Each pixel uses 18 phases:
//    phases 0-8:  3×3 MAC (9 kernel positions)
//    phase  9:    accumulator reduce
//    phases 10-17: requantise + store (8 words)
//  This amortises the pipeline fill (18 cycles) across all pixels
//  instead of paying it per pixel.
// ============================================================
...
#pragma HLS BIND_STORAGE variable=rq_amax_cache  type=ram_1p impl=lutram
    ...
    // Line buffer: 3 input rows × in_w columns × ALL channels.
    // dim=1 complete (3 rows), dim=3 cyclic by DW_PARALLEL so 32
    // channels of any tile can be read in one cycle.
    static int8_t_hls line_buf[3][MAX_W][MAX_C];
#pragma HLS ARRAY_PARTITION variable=line_buf complete dim=1
#pragma HLS ARRAY_PARTITION variable=line_buf cyclic factor=DW_PARALLEL dim=3
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=256
    ...
    // Split into 2x iterations to avoid 2 BRAM reads per cycle (II=1)
    LOAD_DW_REQUANT: for (int idx = 0; idx < in_c * 2; idx++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=2 max=512
        int c = idx >> 1;
        int phase = idx & 1;
        int base_word = ((int)requant_base + c * 8) / 4;
        ap_uint<32> rd = act_in[base_word + phase];
        if (phase == 0) {
            rq_mult_cache[c] = (int32_t_hls)rd;
        } else {
            rq_shift_cache[c] = (int8_t_hls)rd( 7,  0);
            rq_amin_cache[c]  = (int8_t_hls)rd(15,  8);
            rq_amax_cache[c]  = (int8_t_hls)rd(23, 16);
        }
    }
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=48
        ...
        // Load any input rows not yet in the line buffer (all channels)
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=48
                LOAD_CH: for (int c4 = 0; c4 < in_c4; c4++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=64
                    int byte_addr = (int)input_base +
                        ir * in_w * in_c + col * in_c + c4 * 4;
                    ap_uint<32> w = act_in[byte_addr / 4];
                    int c = c4 * 4;
                    line_buf[slot][col][c + 0] = (int8_t_hls)w( 7,  0);
                    line_buf[slot][col][c + 1] = (int8_t_hls)w(15,  8);
                    line_buf[slot][col][c + 2] = (int8_t_hls)w(23, 16);
                    line_buf[slot][col][c + 3] = (int8_t_hls)w(31, 24);
                }
            }
            max_loaded = ir;
        }

        // Merged compute: process all output pixels per channel tile
        // in a single pipeline.  Each pixel = 9 MAC + 1 reduce +
        // DW_PARALLEL/4 store = DW_PHASES_PER_PIX (18) iterations.
        // Pipeline fill (18 cycles) is paid once per c_tile instead
        // of once per pixel, saving ~60% of DW compute cycles.
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=8
            ...
            DW_OW_COMPUTE: for (int idx = 0;
                                idx < out_w * DW_PHASES_PER_PIX;
                                idx++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=18 max=864
                ...
                if (phase <= 8) {
                    ...
                    int iw = ow_cnt * stride + kw - pad_left;
                    bool is_pad = (ih < 0 || ih >= in_h ||
                                   iw < 0 || iw >= in_w);
                    int safe_ih = is_pad ? 0 : ih;
                    int safe_iw = is_pad ? 0 : iw;
                    int slot = safe_ih % 3;
                    int d    = k % ACC_DEPTH;

                    for (int t = 0; t < DW_PARALLEL; t++) {
#pragma HLS UNROLL
                        int8_t_hls in_val = is_pad ? input_zp
                            : line_buf[slot][safe_iw][c_base + t];
                        int32_t_hls product =
                            ((int32_t_hls)in_val - (int32_t_hls)input_zp) *
                            (int32_t_hls)w_cache[k][c_base + t];

                        if (k == 0) {
                            // Init + first MAC combined
                            acc[t][0] = bias_cache[c_base + t] + product;
                            acc[t][1] = (int32_t_hls)0;
                            acc[t][2] = (int32_t_hls)0;
                            acc[t][3] = (int32_t_hls)0;
                        } else {
                            acc[t][d] += product;
                        }
                    }
...
#pragma HLS UNROLL
                    ...
                    // ---------- Store phase (phases 10..17) ----------
                    int t_base = (phase - 10) * 4;
                    ap_uint<32> out_word;
                    for (int b = 0; b < 4; b++) {
#pragma HLS UNROLL
                        int c = c_base + t_base + b;
                        int8_t_hls out_val = (int8_t_hls)0;
                        if (c < in_c) {
                            out_val = requantise(
                                acc[t_base + b][0],
                    ...
                    int byte_addr = (int)output_base +
                        (oh * out_w * in_c) + (ow_cnt * in_c) +
                        c_base + t_base;
                    act_out[byte_addr / 4] = out_word;
                }

                // Advance pixel-phase counter
                if (phase == DW_PHASES_PER_PIX - 1) {
                    phase = 0;
                    ow_cnt++;
                } else {
                    phase++;
